...
namespace KINECT{
	...
	bool docalib = false;
#endif
	bool KinectManagerOpenNI::doCalib(){
		return docalib;
	}

	bool KinectManagerOpenNI::init(){
		return initNI();
	}

	bool KinectManagerOpenNI::release(){
		shutdownNI();
		return true;
	}

	void KinectManagerOpenNI::updateFrames(){
	}

	cv::Mat KinectManagerOpenNI::getColorFrame(){
	...
	CroppedCvMat KinectManagerOpenNI::getPlayerColorFrame(){
		unsigned char buffer[CAPTURE_SIZE_X * CAPTURE_SIZE_Y * 3];
	...
	cv::Mat KinectManagerOpenNI::getDepthFrame(){
	...
	Skeleton KinectManagerOpenNI::getSkeleton(){
	...
	int KinectManagerOpenNI::getCenterJoint(){
		return nite::JOINT_TORSO;
	}

	int KinectManagerOpenNI::getHeadJoint(){
		return nite::JOINT_HEAD;
	}

	void KinectManagerOpenNI::initMapping(Mapping * mapping){
	...
	cv::Vec3f KinectManagerOpenNI::calculateFacing(Skeleton * _s){
	...
	bool KinectManagerOpenNI::checkTracked(int state){
		//TODO
		return true;
	}

	bool KinectManagerOpenNI::skeletonIsGood(){
		nite::Skeleton s;
		return getSkeletonData(&s);
	}

	float KinectManagerOpenNI::getSkeletonGoodness(Skeleton * s){
		float mult = s->states[KINECT::getCenterJoint()];
		if(mult <0.3) mult = 0.3;
		return mult;
	}

	int KinectManagerOpenNI::initSkeletonScore(Skeleton kinectPoints){
	...
	void KinectManagerOpenNI::augmentSkeleton(Skeleton * s){
	...
	void KinectManagerOpenNI::saveParams(std::string filename){
	...
	void KinectManagerOpenNI::loadParams(std::string filename){
		initFromFile(filename);
	}

	//delete later

	void KinectManagerOpenNI::getKinectData_depth_raw(DepthXY * depthPoints){
		if(!init()) { 
	...
	cv::Vec3f KinectManagerOpenNI::mapDepthToSkeletonPoint(DepthXY d){
		cv::Vec3f ret;
	...
	std::pair<int, int> KinectManagerOpenNI::facingHelper(int s){
		if(s==1) //shoulders
		{
	...
	cv::Mat KinectManagerOpenNI::mapCameraPointsToColorPoints(cv::Mat cameraPoints){
		return cv::Mat();
	}
}