...
cv::Mat cylinder_to_pts(unsigned int width, unsigned int height, cv::Vec3f a, cv::Vec3f b, float radius, cv::Point voff, PixelPolygon * p, std::vector<cv::Vec3f> * fromPixels, std::vector<cv::Vec3s> * fromPixels_2d_v, cv::Mat& cameraMatrix);
...
cv::Mat cylinder_to_pts(unsigned int width, unsigned int height, cv::Vec3f a_, cv::Vec3f b_, float radius, cv::Point voff, cv::Rect * r, std::vector<cv::Vec3f> * fromPixels, std::vector<cv::Vec3s> * fromPixels_2d_v, cv::Mat& cameraMatrix);
