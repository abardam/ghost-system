  ...
  : mGLWindow(mVideoSource.Size(), "PTAMM")
{
  ...
  //lock PTAMM camera position
  GUI.ParseLine("LockAR=0");
  GUI.ParseLine("Menu.AddMenuToggle Root \"Lock AR\" LockAR Root");
  ...
  GUI.ParseLine("Menu.ShowMenu Root");
  GUI.ParseLine("Menu.AddMenuButton Root \"Reset All\" ResetAll Root");
  GUI.ParseLine("Menu.AddMenuButton Root \"Reenact\" LoadMap Root");
  GUI.ParseLine("DrawAR=1");
  GUI.ParseLine("LockMap=1");
...
void System::Run()
{
  while(!mbDone)
  {
	...
	mpARDriver->Reset();
      ...
      bool bDrawAR = (mpMap->IsGood()||*gvnLockAR) && *gvnDrawAR;
